...
def run_accuracy_benchmark(cfg: DictConfig) -> float:
    ...
    with mlflow.start_run(run_name=f"accuracy_{engine_config.nfft}x{engine_config.channels}"):
        ...
        #   Format: accuracy_summary_{nfft}_{channels}_{overlap}_{mode}.csv
        #   Example: accuracy_summary_4096_2_0p0000_batch.csv
        #
        # Why this works:
        ...
        overlap_str = f"{engine_config.overlap:.4f}".replace('.', 'p')  # 0.0 -> 0p0000
        summary_path = output_dir / f"accuracy_summary_{engine_config.sample_rate_hz}_{engine_config.nfft}_{engine_config.channels}_{overlap_str}_{exec_mode}.csv"
