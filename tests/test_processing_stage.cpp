 ...
 * @author [Kevin Rahsaz]
 *
 * @brief Unit tests for the individual processing stages (Window, FFT, Magnitude).
 *
 * This suite validates the correctness of each component in the signal processing
 * pipeline, ensuring they initialize correctly and produce the expected output for
 * known signals. It relies on the Google Test framework.
 */
...
TEST_F(ProcessingStageTest, WindowStageProcess) {
    ...
    bool all_same = std::equal(host_input.begin(), host_input.end(), host_output.begin());
...
TEST_F(ProcessingStageTest, FFTStageProcess) {
    ...
    float dc_magnitude = std::sqrt(host_output[0].x * host_output[0].x + host_output[0].y * host_output[0].y);
...
TEST_F(ProcessingStageTest, FFTStageSinusoid) {
    ...
    for (size_t ch = 0; ch < static_cast<size_t>(config_.batch); ++ch) {
        for (size_t i = 0; i < static_cast<size_t>(config_.nfft); ++i) {
            host_input[ch * config_.nfft + i] = std::cos(2.0f * M_PI * freq_bin * i / config_.nfft);
        }
    }
    ...
    for (size_t ch = 0; ch < static_cast<size_t>(config_.batch); ++ch) {
        ...
        for (size_t bin = 0; bin < static_cast<size_t>(config_.nfft / 2 + 1); ++bin) {
...
TEST_F(ProcessingStageTest, MagnitudeStageProcess) {
    ...
    std::vector<float2> host_input(total_complex, {3.0f, 4.0f}); // magnitude should be 5
...
TEST_F(ProcessingStageTest, MagnitudeStageScaling) {
    ...
    const size_t num_bins = config_.nfft / 2 + 1;
    const size_t total_complex = num_bins * config_.batch;
    std::vector<float2> host_input(total_complex, {static_cast<float>(config_.nfft), 0.0f});
...
TEST_F(ProcessingStageTest, FullPipelineIntegration) {
    ...
    // This corrected implementation generates a proper signal for each batch item,
    // ensuring the frequency content is correct for the FFT length.
    std::vector<float> host_input(total_samples);
    for (size_t ch = 0; ch < static_cast<size_t>(config_.batch); ++ch) {
        for (size_t i = 0; i < static_cast<size_t>(config_.nfft); ++i) {
    ...
    mag_stage.process(d_fft.get(), d_magnitude.get(), (config_.nfft / 2 + 1) * config_.batch, stream_->get());
