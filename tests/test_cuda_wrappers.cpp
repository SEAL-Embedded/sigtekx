 ...
 * @brief Verifies that a CudaStream object is created, is valid, and can be synchronized.
 */
 ...
 * @brief Validates the move constructor and move assignment operator for CudaStream.
 */
 ...
 * @brief Verifies that a CudaEvent object is created, can be recorded, and synchronized.
 */
 ...
 * @brief Tests direct element access on a pinned host buffer. This test was restored.
 */
 ...
 * @brief Verifies memset functionality for pinned host buffers. This test was restored.
 */
...
TEST_F(CudaWrappersTest, CufftPlanCreation) {
    ...
    EXPECT_NO_THROW(plan.create_plan_many(1, n, nullptr, 1, nfft, nullptr, 1, nfft, CUFFT_C2C, batch, stream.get()));
...
TEST_F(CudaWrappersTest, CufftPlanExecute) {
    ...
    plan.create_plan_many(1, n, nullptr, 1, nfft, nullptr, 1, complex_size, CUFFT_R2C, batch, stream.get());
    ...
    EXPECT_NO_THROW(plan.exec_r2c(reinterpret_cast<cufftReal*>(real_data.get()), reinterpret_cast<cufftComplex*>(complex_data.get())));
    stream.synchronize();
}
...
TEST_F(CudaWrappersTest, StreamEventSynchronization) {
    ...
    IONO_CUDA_CHECK(cudaMemcpyAsync(buffer2.get(), buffer1.get(), buffer1.bytes(), cudaMemcpyDeviceToDevice, stream2.get()));
