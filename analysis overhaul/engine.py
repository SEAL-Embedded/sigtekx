...
class AnalysisEngine:
    ...
    def analyze_scaling(
        ...
        if metrics is None:
            ...
            for col in data.columns:
                if any(keyword in col for keyword in ['latency', 'fps', 'throughput', 'rate']):
                    metrics.append(col)
    ...
    def generate_summary(
        ...
        for (nfft, channels), _ in data.groupby(['engine_nfft', 'engine_channels']):
            configs.append(EngineConfiguration(nfft=int(nfft), channels=int(channels)))
        ...
        for bench_type in benchmark_types:
            ...
            for config_key, config_analysis in analysis_results.items():
                ...
                result = BenchmarkResult(
                    ...
                    engine_config=EngineConfiguration(nfft=nfft, channels=channels),
        ...
        if BenchmarkType.LATENCY in results_by_type:
            ...
            if 'mean_latency_us' in latency_data.columns:
                ...
                if optimal:
                    optimal_configs[BenchmarkType.LATENCY.value] = EngineConfiguration(**optimal['configuration'])
        ...
        if BenchmarkType.THROUGHPUT in results_by_type:
            ...
            if 'frames_per_second' in throughput_data.columns:
                ...
                if optimal:
                    optimal_configs[BenchmarkType.THROUGHPUT.value] = EngineConfiguration(**optimal['configuration'])
