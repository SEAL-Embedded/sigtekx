...
class EngineConfiguration(BaseModel):
    ...
    def total_samples(self) -> int:
        ...
        return self.nfft * self.channels
    
    @property
    def memory_footprint_mb(self) -> float:
        """Estimated memory footprint in MB."""
        # Complex float32: 8 bytes per sample
        # Factor of 2 for input/output buffers
        return (self.total_samples * 8 * 2) / (1024 * 1024)
    
    @property
    def compute_complexity(self) -> float:
        """Relative computational complexity (FFT operations)."""
        return self.channels * self.nfft * np.log2(self.nfft)
...
class ExperimentSummary(BaseModel):
    ...
    configurations_tested: List[EngineConfiguration]
    ...
    # Key findings
    optimal_configs: Dict[str, EngineConfiguration]
