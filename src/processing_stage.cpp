 ...
 * @author [Kevin Rahsaz]
 *
 * @brief Implements the concrete processing stages for the signal processing pipeline.
 *
 * This file provides the implementations for the Window, FFT, and Magnitude stages
 * declared in processing_stage.hpp. It uses the Pimpl idiom to hide internal
 * implementation details (like cuFFT plans and device buffers) from the header,
 * promoting cleaner architecture and faster compile times.
 */
...
public:
    ...
    void initialize(const StageConfig& config, cudaStream_t stream) {
        ...
        // Ensure the window is fully uploaded before subsequent stages might use it.
...
void WindowStage::initialize(const StageConfig& config, cudaStream_t stream) { pImpl->initialize(config, stream); }
void WindowStage::process(void* input, void* output, size_t num_samples, cudaStream_t stream) { pImpl->process(input, output, num_samples, stream); }
size_t WindowStage::get_workspace_size() const { return pImpl->get_workspace_size(); }
...
void FFTStage::initialize(const StageConfig& config, cudaStream_t stream) { pImpl->initialize(config, stream); }
void FFTStage::process(void* input, void* output, size_t num_samples, cudaStream_t stream) { pImpl->process(input, output, num_samples, stream); }
size_t FFTStage::get_workspace_size() const { return pImpl->get_workspace_size(); }
...
public:
    ...
    void process(void* input, void* output, size_t num_elements, cudaStream_t stream) {
        ...
        if (num_elements % static_cast<size_t>(frames) != 0) {
            throw std::runtime_error("MagnitudeStage: num_elements not divisible by batch size");
        }
        
        const int inferred_stride = static_cast<int>(num_elements / static_cast<size_t>(frames));
        ...
        kernels::launch_magnitude(
            ...
            bins_per_frame, frames, inferred_stride, scale_, stream
        );
    }
...
void MagnitudeStage::initialize(const StageConfig& config, cudaStream_t stream) { pImpl->initialize(config, stream); }
void MagnitudeStage::process(void* input, void* output, size_t num_samples, cudaStream_t stream) { pImpl->process(input, output, num_samples, stream); }
size_t MagnitudeStage::get_workspace_size() const { return pImpl->get_workspace_size(); }
...
std::unique_ptr<IProcessingStage> StageFactory::create(StageType type) {
    switch (type) {
        ...
        default:
            throw std::invalid_argument("Unknown stage type requested from StageFactory");
    }
}
