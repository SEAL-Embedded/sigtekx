...
static inline nvtx3::v1::argb to_argb(uint32_t argb) {
    return nvtx3::v1::argb{
        static_cast<unsigned char>((argb >> 24) & 0xFF),
...
struct ScopedRange::Impl {
    ...
    explicit Impl(const char* name, uint32_t color)
        : range(nvtx3::v1::event_attributes{ nvtx3::v1::message{name}, to_argb(color) }) {}
};
...
void nvtx_mark(const char* message, uint32_t color) {
    if (g_profiling_enabled) {
        nvtx3::v1::mark(nvtx3::v1::event_attributes{ nvtx3::v1::message{message}, to_argb(color) });
    }
}
