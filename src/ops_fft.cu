 ...
 * designed for high performance, adhering to best practices like coalesced memory
 * access and grid-stride loops for scalability, which are essential for
 ...
 * The grid-stride loop pattern (`for (int idx = ...; idx < total; idx += gridDim.x * blockDim.x)`)
 * is used to ensure that kernels are flexible and can process data of any
 * size, regardless of the launch grid dimensions.
 */
...
/** @brief The number of threads in a CUDA warp, a fundamental unit of execution on NVIDIA GPUs. */
constexpr int WARP_SIZE = 32;
/** @brief The maximum number of threads per block, chosen as a common value for good hardware occupancy. */
 ...
 * window coefficient. It can operate in-place (input == output) or out-of-place.
 *
 * @param[in]  input       Device pointer to the input real-valued signal.
 * @param[out] output      Device pointer to the windowed output real-valued signal.
 ...
 * @param stride           The distance (in elements) between the start of consecutive signals.
 */
 ...
 * @param[out] output       Device pointer for the complex-valued output signal (float2).
 * @param nfft              The size of the FFT frame.
 * @param batch             The number of signals in the batch.
...
__global__ void window_and_convert_kernel(const float* __restrict__ input,
    ...
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; 
        ...
        output[channel_idx * nfft + sample_idx] = make_float2(sample * window_val, 0.0f);
    }
}
 ...
 *
 * @param[in]  input    Device pointer to the packed complex FFT output (float2).
 ...
 * @param batch         The number of signals in the batch.
 * @param scale         A scaling factor to apply to the magnitude (e.g., 1.0/nfft).
 */
 ...
 *
 * @param[in]  input        Device pointer to the strided complex FFT output (float2).
 * @param[out] output       Device pointer for the tightly packed magnitude data (float).
 * @param num_bins          The number of complex frequency bins per signal.
 * @param batch             The number of signals in the batch.
 * @param input_stride      The distance (in elements) between the start of consecutive FFTs in the input buffer.
 ...
 * @param num_bins      The number of complex frequency bins per signal.
 * @param batch         The number of signals in the batch.
 * @param scale         A scaling factor to apply. The final result is scaled by `scale^2`.
 */
__global__ void magnitude_squared_kernel(const float2* __restrict__ input,
    ...
    for (int idx = blockIdx.x * blockDim.x + threadIdx.x; 
         ...
         idx += blockDim.x * gridDim.x) {
        const float2 complex_val = input[idx];
        output[idx] = (complex_val.x * complex_val.x + complex_val.y * complex_val.y) * scale * scale;
    }
}
 ...
 * @param batch          The number of signals in the batch.
 * @param ref_level      The reference level for the dB calculation (typically 1.0).
...
void launch_apply_window(const float* input, float* output, const float* window,
    ...
    apply_window_kernel<<<blocks, threads, 0, stream>>>(input, output, window, nfft, batch, stride);
}
...
void launch_real_to_complex(const float* input, float2* output,
    ...
    const int threads = std::min(MAX_THREADS_PER_BLOCK, total_elements);
    const int blocks = (total_elements + threads - 1) / threads;
    real_to_complex_kernel<<<blocks, threads, 0, stream>>>(input, output, nfft, batch, stride);
}
...
void launch_window_and_convert(const float* input, float2* output, const float* window,
    ...
    const int threads = std::min(MAX_THREADS_PER_BLOCK, total_elements);
    const int blocks = (total_elements + threads - 1) / threads;
    window_and_convert_kernel<<<blocks, threads, 0, stream>>>(input, output, window, nfft, batch, stride);
}
...
void launch_magnitude(const float2* input, float* output, int num_bins, int batch,
    ...
    const int threads = std::min(MAX_THREADS_PER_BLOCK, total_elements);
    const int blocks = (total_elements + threads - 1) / threads;
    magnitude_kernel<<<blocks, threads, 0, stream>>>(input, output, num_bins, batch, scale);
}
...
void launch_magnitude(const float2* input, float* output,
    ...
    const int threads = std::min(MAX_THREADS_PER_BLOCK, total_elements);
    const int blocks = (total_elements + threads - 1) / threads;
    magnitude_strided_kernel<<<blocks, threads, 0, stream>>>(input, output, num_bins, batch, input_stride, scale);
}
...
void launch_magnitude_squared(const float2* input, float* output, int num_bins, int batch,
    ...
    const int threads = std::min(MAX_THREADS_PER_BLOCK, total_elements);
    const int blocks = (total_elements + threads - 1) / threads;
    magnitude_squared_kernel<<<blocks, threads, 0, stream>>>(input, output, num_bins, batch, scale);
}
