 ...
 * ensures the public `ResearchEngine` header remains clean of CUDA dependencies.
 */
class ResearchEngine::Impl {
public:
    /**
    ...
    /**
     * @brief Initializes all engine resources based on the provided configuration.
     *
     * This function is the main setup routine. It creates CUDA streams and events,
     * allocates device and pinned host memory, and initializes the processing stages.
     *
    ...
    void process(const float* input, float* output, size_t num_samples) {
        if (!initialized_) {
            throw std::runtime_error("ResearchEngine is not initialized. Call initialize() first.");
        }
    ...
    /**
     * @brief Asynchronously processes data and invokes a callback upon completion.
     */
    ...
    /**
     * @brief Blocks until all streams in the engine have completed all pending work.
     */
    void synchronize() {
    ...
    RuntimeInfo get_runtime_info() const {
        ...
        info.cufft_version = info.cuda_version; // cuFFT version often tracks CUDA version
    ...
    void add_stage(std::unique_ptr<IProcessingStage> stage) { stages_.push_back(std::move(stage)); }
...
void ResearchEngine::dump_profiling_data(const std::string&) { /* NVTX integration placeholder */ }
...
namespace engine_utils {
        ...
        if (cudaGetDeviceCount(&device_count) == cudaSuccess) {
            for (int i = 0; i < device_count; ++i) {
                ...
                if (cudaGetDeviceProperties(&prop, i) == cudaSuccess) {
                    ...
                    oss << "[" << i << "] " << prop.name << " (CC " << prop.major << "." << prop.minor << ")";
    ...
    size_t estimate_memory_usage(const EngineConfig& cfg) {
        size_t total = 0;
        const size_t input_bytes = static_cast<size_t>(cfg.nfft) * cfg.batch * sizeof(float);
        const size_t output_bytes = static_cast<size_t>(cfg.num_output_bins()) * cfg.batch * sizeof(float);
        const size_t complex_bytes = output_bytes * 2;

        total += cfg.pinned_buffer_count * (input_bytes + output_bytes + complex_bytes);
