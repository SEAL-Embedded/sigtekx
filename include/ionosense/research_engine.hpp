 ...
 * @author [Kevin Rahsaz]
 *
 * @brief Public API for the Ionosense High-Performance Computing (HPC) Research Engine.
 *
...
struct EngineConfig {
    ...
    int batch = 2;                  ///< Number of signals processed in parallel (e.g., dual-channel antenna).
    ...
    int pinned_buffer_count = 2;    ///< Number of pinned host buffers (e.g., for double-buffering).
    int warmup_iters = 1;           ///< Number of warmup iterations to stabilize GPU clocks.
    int timeout_ms = 1000;          ///< Timeout for asynchronous operations (in milliseconds).
    
    // --- Performance Tuning ---
    bool use_cuda_graphs = false;   ///< If true, enables CUDA Graphs for low-overhead kernel launches. (Future feature)
    bool enable_profiling = false;  ///< If true, enables internal performance metric collection.
    
    /**
     * @brief Computes the hop size (samples between frames) based on nfft and overlap.
...
/**
 * @struct RuntimeInfo
 * @brief Contains information about the CUDA runtime environment and GPU device.
 *
...
struct RuntimeInfo {
    ...
    std::string device_name;                    ///< Name of the GPU device (e.g., "NVIDIA RTX 4000 Ada").
    int device_compute_capability_major;    ///< Major compute capability of the device.
    int device_compute_capability_minor;    ///< Minor compute capability of the device.
...
public:
    ...
    virtual void process_async(const float* input, size_t num_samples, ResultCallback callback) = 0;
 ...
 * for research and development. It uses a configurable pipeline of processing stages.
 */
...
public:
    ...
    void process_async(const float* input, size_t num_samples, ResultCallback callback) override;
...
private:
    // Pimpl idiom hides implementation details (CUDA headers, etc.) from this public header.
    class Impl;
...
namespace engine_utils {
    ...
    /**
     * @brief Selects the best available CUDA device based on a performance heuristic.
     ...
     * @param[out] error_msg A string to receive an error message if validation fails.
