 ...
 * @author [Kevin Rahsaz]
 *
 * @brief Provides robust, RAII-compliant C++ wrappers for CUDA and cuFFT resources.
 *
 ...
 * reproducible engineering (RE). The documentation style follows IEEE standards.
 */
 ...
 * automatically upon destruction. This class is move-only to enforce unique ownership.
 */
class CudaStream {
public:
    /**
     ...
     */
    explicit CudaStream(cudaStream_t stream) noexcept : stream_(stream), owned_(false) {}
    ...
    ~CudaStream() {
        if (stream_ && owned_) {
            ...
            // In case of an error, it is ignored to comply with no-throw destructor guidelines.
    ...
    /**
     * @brief Synchronizes the stream. Blocks the host until all preceding commands in the stream are complete.
     */
     ...
     * @return True if all preceding commands in the stream have completed, false otherwise.
     */
    bool query() const {
...
public:
    /**
     * @brief Default constructor. Creates a new event with timing disabled for performance.
     */
     ...
     * @param stream The stream to record the event in. Defaults to the legacy default stream (0).
     */
...
public:
    ...
    explicit PinnedHostBuffer(size_t count) : ptr_(nullptr), size_(0) {
        if (count > 0) {
            size_ = count;
            IONO_CUDA_CHECK(cudaHostAlloc(&ptr_, count * sizeof(T), cudaHostAllocDefault));
        }
    }
    ...
    void resize(size_t new_count) {
        if (new_count != size_) {
            ...
            size_ = new_count;
            if (size_ > 0) {
                IONO_CUDA_CHECK(cudaHostAlloc(&ptr_, size_ * sizeof(T), cudaHostAllocDefault));
            }
        }
    }
 ...
 *
 * Manages the lifetime of a cuFFT plan, including its work area. This class is move-only.
 */
class CufftPlan {
public:
    ...
    /**
     * @brief Creates a many-transform cuFFT plan with proper work area management.
     *
     * This method encapsulates the creation, work-area querying, and stream association
     * for a cufftPlanMany operation. It disables auto-allocation to manage the work area buffer
     * explicitly, which is a best practice for performance and graph compatibility.
     *
    ...
    void create_plan_many(int rank, int* n, int* inembed, int istride, int idist,
        ...
        IONO_CUFFT_CHECK(cufftMakePlanMany(plan_, rank, n, inembed, istride, idist,
                                       onembed, ostride, odist, type, batch, &work_size_));
