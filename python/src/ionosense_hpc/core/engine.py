...
from ..config import EngineConfig, validate_input_array, validate_batch_size
from ..exceptions import EngineStateError, ValidationError
from ..utils.logging import logger, log_config, log_performance
...
class Engine:
    ...
    def process_frames(
        ...
        if len(input_data) < frame_size:
            raise ValidationError(
                f"Input too short for even one frame",
