 ...
 * prevent NaN/Inf propagation, which is critical for reliable signal
 * processing.
 */
...
TEST(WindowFunctionsTest, RectangularInvalidParams) {
  ...
  EXPECT_THROW(base(WindowKind::RECTANGULAR, 1024, 1024),
  ...
  for (int size : sizes) {
    for (int i = 0; i < size; ++i) {
      ...
      EXPECT_TRUE(std::isfinite(value))
          << "Non-finite at size=" << size << ", i=" << i;
      EXPECT_GE(value, -0.1)
          << "Unexpectedly negative at size=" << size << ", i=" << i;
      EXPECT_LE(value, 1.1)
          << "Unexpectedly large at size=" << size << ", i=" << i;
    }
  }
}
...
TEST(WindowFunctionsTest, WindowValueFiniteOutputs) {
  const std::vector<int> sizes{1, 2, 16, 64, 256, 1024};
  const std::vector<WindowKind> kinds{WindowKind::RECTANGULAR, WindowKind::HANN,
                                      WindowKind::BLACKMAN};
...
TEST(WindowFunctionsTest, LargeWindowSize) {
  ...
  const std::vector<int> sample_indices{0,
                                        1,
                                        large_size / 4,
                                        large_size / 2,
                                        3 * large_size / 4,
 ...
 * @brief Verifies that Hann window has correct boundary values in SYMMETRIC
 * mode.
 *
...
TEST(WindowFunctionsTest, HannBoundaryValues) {
  ...
  EXPECT_NEAR(first, 0.0, 1e-6)
      << "Hann window (SYMMETRIC) should be ~0 at start";
 ...
 */
TEST(WindowFunctionsTest, FillWindowNullPointer) {
  EXPECT_THROW(fill_window(nullptr, 64, WindowKind::HANN, false),
               std::invalid_argument);
}
...
TEST(WindowFunctionsTest, FillWindowInvalidSize) {
  ...
  EXPECT_THROW(fill_window(buffer.data(), 0, WindowKind::HANN, false),
               std::invalid_argument);
  EXPECT_THROW(fill_window(buffer.data(), -1, WindowKind::HANN, false),
               std::invalid_argument);
}
...
TEST(WindowFunctionsTest, FillWindowValidOperation) {
  ...
  for (int i = 0; i < size; ++i) {
    EXPECT_TRUE(std::isfinite(buffer[i])) << "Non-finite value at index " << i;
  }
...
TEST(WindowFunctionsTest, FillWindowWithSqrtNorm) {
  ...
  for (int i = 0; i < size; ++i) {
    EXPECT_TRUE(std::isfinite(buffer[i])) << "Non-finite value at index " << i;
    EXPECT_GE(buffer[i], 0.0f) << "Negative value at index " << i;
  }
}
 ...
 * @brief Verifies PERIODIC mode produces correct results (default for FFT
 * processing).
 *
 ...
 * @brief Verifies SYMMETRIC mode produces correct results (for signal
 * analysis).
 *
...
TEST(WindowFunctionsTest, WindowSymmetryModes) {
  ...
  fill_window(periodic_buffer.data(), size, WindowKind::HANN, false,
              WindowSymmetry::PERIODIC);
  fill_window(symmetric_buffer.data(), size, WindowKind::HANN, false,
              WindowSymmetry::SYMMETRIC);
  ...
  EXPECT_TRUE(buffers_differ) << "fill_window() should produce different "
                                 "results for different symmetry modes";
}
