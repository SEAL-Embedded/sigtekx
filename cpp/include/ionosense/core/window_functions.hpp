 ...
 *
 * This header provides robust window function implementations with
 * comprehensive error handling and numerical stability guarantees:
 ...
 * in coefficient calculations, affecting endpoint values and spectral
 * characteristics:
 *
 ...
 *
 * **Default**: All functions default to PERIODIC mode as this library is
 * designed for FFT-based ionosphere research workflows.
 *
 * @see StageConfig::WindowSymmetry in processing_stage.hpp for usage in
 * pipeline stages
 */
 ...
 * @param symmetry Window symmetry mode (see module-level documentation).
 * @return The Hann window coefficient, or NaN (GPU) / throws exception (CPU) on
 * error.
 *
 * Error conditions:
...
IONO_HD inline double hann_base(
  ...
  if (size <= 0 || index < 0 || index >= size) {
    ...
    throw std::invalid_argument(
        "Invalid window parameters: size=" + std::to_string(size) +
        ", index=" + std::to_string(index));
#endif
  }

  const double ratio = (2.0 * PI * static_cast<double>(index)) /
                       safe_denominator(size, symmetry);
 ...
 * @param size The total window size.
 * @param symmetry Window symmetry mode (see module-level documentation).
 * @return The Blackman window coefficient, or NaN (GPU) / throws exception
 * (CPU) on error.
 *
 * Error conditions:
 * - size <= 0: Invalid window size
...
IONO_HD inline double blackman_base(
  ...
  const double ratio =
      static_cast<double>(index) / safe_denominator(size, symmetry);
...
/**
 * @brief Computes the final window value with optional square-root
 * normalization.
 * @param kind The type of window function.
 * @param index The sample index within the window.
 * @param size The total window size.
...
IONO_HD inline float window_value(
  ...
  if (!std::isfinite(adjusted)) {
    throw std::runtime_error(
        "Window function produced non-finite value after normalization");
  }
#endif
 ...
 */
inline void fill_window(float* destination, int size, WindowKind kind,
                        bool sqrt_norm,
                        WindowSymmetry symmetry = WindowSymmetry::PERIODIC) {
