...
- `./scripts/cli.sh build [preset]` or `sigx build`: configure and build via CMake presets.
- `./scripts/cli.sh format` or `sigx format`: run clang-format checks (Google style).
- `./scripts/cli.sh lint` or `sigx lint`: run Python lint plus C++ static analysis.
- `./scripts/cli.sh typecheck`: strict mypy across `src/sigtekx/` and `tests/`.
- `./scripts/cli.sh test`, `./scripts/cli.sh test py`, `./scripts/cli.sh test cpp`: run unit and integration suites.
- `./scripts/cli.sh bench suite`, `sxp nsys <target>`: benchmarking and profiling flows.
...
- **Python**: 3.11.x via the `sigtekx` conda environment.
...
- Outputs belong under `build/` (default via `SIGX_OUTPUT_ROOT`); do not commit generated results.
